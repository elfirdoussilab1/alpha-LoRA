...
def linear_scheduler(start_value, end_value, total_epochs):
    def scheduler(epoch):
        # Clamp epoch to [0, total_epochs]
        epoch = min(max(epoch, 0), total_epochs)
...
def train(model, loader, args):
    ...
    scheduler = linear_scheduler(start_value=args.alpha, end_value=0.5, total_epochs=args.n_epochs)
    for epoch in range(args.n_epochs):
        ...
        # New alpha
        alpha = scheduler(epoch)
        change_alpha(model, alpha)
        ...
        for i, batch in enumerate(tqdm(loader['train'], desc=f"Epoch {epoch+1}/{args.n_epochs}")):
            if i % args.inter_eval == 0 or i == n: 
                ...
                if evals["test_acc"] > best_acc:
                    ...
                    path = f'./models/{args.model_name}_sentiment_alpha_scheduler_{args.alpha}.pth'
...
if __name__ == "__main__":
    ...
    wandb.init(
        ...
        name = f'alpha_scheduler_{round(args.alpha, 3)}'
    )
